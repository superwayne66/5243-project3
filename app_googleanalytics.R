...
read_uploaded_file <- function(file_path, file_name) {
  ...
  switch(ext,
         ...
         "txt"  = data.frame(
           line_number = seq_along(readLines(file_path, warn = FALSE, encoding = "UTF-8")),
           text = readLines(file_path, warn = FALSE, encoding = "UTF-8"),
           stringsAsFactors = FALSE
         ),
         stop(paste0("Unsupported file format: .", ext,
                     ". Please upload CSV, Excel, JSON, RDS, or TXT."))
  )
}

read_txt_content <- function(file_path) {
...
ui <- tagList(
  ...
  tags$head(
    ...
    # ---- Google Analytics (GA4) base script ----
    tags$script(async = NA, src = "https://www.googletagmanager.com/gtag/js?id=G-XXXXXXXXXX"),
    tags$script(HTML("
  ...
  ")),
    ...
    # ---- Custom event handler (connects Shiny → GA) ----
    tags$script(HTML("
    Shiny.addCustomMessageHandler('ga_event', function(x) {
      if (typeof gtag !== 'undefined') {
        gtag('event', x.event_name, {
          condition: x.condition,
          dataset: x.dataset,
          tab: x.tab,
          seconds_from_start: x.seconds_from_start
        });
      }
    });
  "))
      ...
      fluidRow(
        column(3,
               div(class = "upload-section",
                   ...
                   fileInput(
                     "file_upload", NULL,
                     accept = c(".csv", ".xlsx", ".xls", ".json", ".rds", ".txt"),
                     placeholder = "CSV, Excel, JSON, RDS, or TXT"
                   ),
                   helpText("Supported: .csv, .xlsx, .xls, .json, .rds, .txt", br(), "Max size: 30 MB"),
  ...
  output$text_editor_ui <- renderUI({
    ...
    div(
      ...
      textAreaInput(
        ...
        width = "100%",
        height = "420px",
        placeholder = "Edit your .txt file here..."
      ),
      div(
        class = "d-flex gap-2",
